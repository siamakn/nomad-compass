...
async def build(req: BuildRequest | None = Body(default=None)):
    ...
    with TemporaryDirectory() as td:
        tmp = Path(td)
        rdf_dir = await download_extract_rdf(req.rdf_zip or DEFAULT_RDF_ZIP, tmp)
    ...
    return {'ok': True, 'docs': count, 'chunks': len(chunks)}


@app.post('/ask')
async def ask(req: AskRequest):
    if app.state.vectors is None or app.state.chunks is None or app.state.metas is None:
        raise HTTPException(
            status_code=400, detail='Index not found. Build first via POST /build.'
        )
